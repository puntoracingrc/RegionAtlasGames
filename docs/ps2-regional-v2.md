...
Las fichas incluyen códigos, idiomas por función, fecha documentada, soporte, jugadores, memoria, periféricos, galerías y ediciones relacionadas cuando hay evidencia. Los campos desconocidos permanecen desconocidos. Las portadas mantienen las rutas `/catalog-covers/ps2/galeria/` y nombres basados en el título y el código. Esas rutas redirigen al almacén público de portadas de RegionAtlas. Los originales también se conservan en Git, dentro de `artifacts/ps2-region-migration/original-covers/`, con su procedencia y SHA-256 en el manifiesto.
...
Los 8.811 originales suman 4.778.168.697 bytes. Se incorporaron a Git en catorce commits de imágenes, de menos de 350 MB cada uno, con los despliegues de la rama desactivados durante los lotes. El archivo permanece versionado fuera de `public/`, para que cada despliegue de la aplicación no vuelva a empaquetar todas las imágenes. La publicación de la integración requiere CI, despliegue y comprobación de la web pública. El estado final se registra en el paquete de entrega de la tarea.

## Publicación de portadas

Vercel señaló un probable agotamiento de disco al empaquetar 8.039 MB de salida. Liberar su clon Git temporal redujo el uso de disco, pero no resolvió la publicación de todos los estáticos. Las imágenes PS2 se sirven desde `regionatlas-covers`, un almacén público de Vercel Blob en Dublín (`store_rcu58iNCyX2S3RIb`). El almacén privado de la aplicación conserva su configuración y sus datos. Las imágenes ya alojadas por SFTP tampoco se trasladan.

`scripts/ps2-regional/publish_covers.mjs` realiza la subida explícita fuera del build. Valida el original local, prohíbe sobrescrituras y descarga cada imagen pública para comprobar tamaño, tipo y SHA-256. Mantiene un diario reanudable. `cover-publication.json.gz` contiene los recibos de verificación de los 8.811 archivos, y `data/ps2-cover-hosting.json` fija el origen público y el hash del manifiesto. La auditoría offline comprueba que cada recibo corresponde al mismo original.

La conexión del almacén añade únicamente `COVERS_READ_WRITE_TOKEN` al entorno development para la publicación administrativa; las 84 variables preexistentes se compararon y conservaron. La web lee imágenes públicas y no necesita esa credencial. El uploader recibe un archivo de credenciales temporal protegido; no contiene secretos en Git ni se ejecuta automáticamente. El consumo de almacenamiento y transferencia pertenece al plan Vercel vigente, según sus [tarifas de Blob](https://vercel.com/docs/vercel-blob/usage-and-pricing).

El comando de compilación libera únicamente `.git` dentro de una copia desechable `/vercel/pathN` en Linux, con `VERCEL=1`, entorno preview/production y SHA de origen verificado. No ejecuta esa limpieza en un checkout local, un worktree, una ruta enlazada ni un commit distinto. El archivo Git local y remoto conserva todos los originales. Los controles de ese límite se ejecutan en `scripts/test-prepare-vercel-build.mjs`.
