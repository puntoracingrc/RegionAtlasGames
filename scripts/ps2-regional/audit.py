"""Resolve every existing PS2 row and enumerate documented missing releases.
...
def group_sources(pages):
    ...
    for identity, members in grouped.items():
        sid = "ps2-release-" + hashlib.sha256(identity.encode()).hexdigest()[:16]
...
def main():
    ...
    for rid, group in groups.items():
        ...
        for sid in group["sourceIds"]:
            for b in by_id[sid].get("barcodes", []):
                ...
                if digits and b.get("checksumValid") and "barcode_source_disclaims_certainty" not in by_id[sid].get("sourceReviewFlags", []):
    ...
    for game in baseline["catalog"]:
        ...
        if invalid_reference:
            reasons.append("legacy_reference_is_not_a_ps2_serial")
        if chosen:
        ...
        blockers = [r for r in reasons if r != "legacy_reference_is_not_a_ps2_serial"]
    ...
    for release_ids in components.values():
        release_ids.sort()
        wid = "ps2-work-" + hashlib.sha256("|".join(release_ids).encode()).hexdigest()[:16]
        works[wid] = {"id": wid, "platformSlug": "ps2", "title": groups[release_ids[0]]["title"], "releaseIds": release_ids, "catalogIds": [], "relationshipBasis": "documented_exact_title_aliases_and_reciprocal_regional_links", "creditsInherited": False}
        for rid in release_ids:
            work_for_release[rid] = wid
    for d in decisions:
        ids = [d["releaseId"]] if d["releaseId"] else d["candidateReleaseIds"]
        if not ids:
            ids = [rid for rid in by_title.get(title_key(d["title"]), set()) if groups[rid]["family"] == d["family"] and title_key(d["title"]) in groups[rid]["workTitleKeys"]]
