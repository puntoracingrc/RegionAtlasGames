"""Apply the frozen PS2 audit in an isolated checkout, preserving all old URLs."""
...
from reference import ART, AT, ROOT, RELEASE_CATEGORIES, evidence, load_gzip, normalized, market
...
def dump(path, value):
    temporary = path.with_name(path.name + ".ps2-v2-tmp")
...
def main():
    ...
    # Re-runs replace only this migration's PS2 projection from the immutable
    # baseline. Any unrelated PS2 edit causes a refusal, never silent clobbering.
    ...
    catalog = [g for g in current_catalog if g["platformSlug"] != "ps2"]
    old_ids = {g["id"] for g in current_catalog if g["platformSlug"] == "ps2"}
    details = {k: v for k, v in current_details.items() if k not in old_ids}
    ps2 = copy.deepcopy(baseline["catalog"])
    ...
    def graphic_references(group):
        ...
        for sid in group["sourceIds"]:
            for asset, ref in assets_by_source[sid]:
                ...
                if archive and (archive.get("publicArchiveUrl") or "").startswith("/catalog-covers/ps2/"):
                    value.update({"url": archive["publicArchiveUrl"], "sha256": archive["sha256"], "bytes": archive["bytes"], "width": archive["width"], "height": archive["height"], "stored": True})
                else:
                    value["stored"] = False
                value["thumbnailOnly"] = ref.get("thumbnailOnly", False) or bool(archive and archive.get("relationshipToReference") == "source_linked_thumbnail_fallback")
                value["layout"] = "full_cover_candidate" if archive and archive.get("width", 0) > 1.25 * archive.get("height", 1) else "portrait_or_square"
                value["visualObservation"] = asset.get("visualObservation")
                value["identifierDifference"] = ref.get("filenameIdentifierDifference")
                ...
                for adjudication in adjudications.values():
                    if value.get("sha256") == adjudication.get("imageSha256") and adjudication.get("packagingMarket"):
                        value["marketHints"] = [market(adjudication["packagingMarket"])["code"]]
    ...
    def enrich(game, decision, action):
        ...
        if group and resolved:
            ...
            page = sorted(page_list, key=lambda p: (p.get("indexOnly", False), p["sourceUrl"]))[0]
            ...
            profile["sourceWarnings"] = sorted(set(profile["sourceWarnings"] + page.get("sourceReviewFlags", [])))
            profile["accessoryCodes"] = page.get("accessoryCodeEvidence", [])
            profile["productCodes"] = page.get("productCodeEvidence", [])
            for key in ["Media", "Video Modes", "Audio Modes", "Special Controllers Included Or Supported ( Official Only )", "Network Adaptor (Ethernet) Compatible", "Language Selector"]:
                value = clean_field(page, key)
                if value:
                    profile["softwareMetadata"][key] = value
            ...
            for a in profile["graphics"]:
                ...
                edition_ok = not re.search(r"platinum|greatest hits|best|mega hits|limited|collector|special|reprint|rerelease", a.get("label") or "", re.I) if not group["labels"] else all(re.sub(r"[^a-z0-9]", "", label) in re.sub(r"[^a-z0-9]", "", (a.get("label") or "").lower()) for label in group["labels"])
                if "front_cover" in a["roles"] and code_in_filename and same_market and clean_group and edition_ok and a["stored"] and not a.get("identifierDifference") and not a.get("thumbnailOnly"):
                    front.append(a)
            if len(front) == 1:
                candidate = front[0]
            ...
            media = clean_field(page, "Media")
            fields = {"support": media} if media else {}
            ...
            if date and re.fullmatch(r"20[0-2]\d-\d\d-\d\d", date) and not group["labels"]:
    ...
    for rid in audit["missingReleaseIds"]:
        ...
        gid = "ps2-" + region["code"].lower() + "-" + first_serial.lower() + suffix
        if gid in by_game:
            gid += "-" + rid[-8:]
        assert gid not in by_game
        game = {"id": gid, "slug": slug(group["title"]) + "-" + first_serial.lower() + suffix, "title": group["title"], "titlePc": None, "platformSlug": "ps2", "region": region["label"], "edition": "standard", "listingStatus": "listed", "coverUrl": None, "pcId": None, "pcPath": None, "pcRegion": None, "pcCondition": None, "matchConfidence": "DOCUMENTED_PS2_REGIONAL_RELEASE", "marketMin": None, "marketMax": None, "recommendedPrice": None, "pcRefPrice": None, "deltaEsVsPc": None, "priceSource": None, "updatedAt": AT, "hasEsPrice": False, "seedSource": "ps2-regional-v2"}
        game["canonicalSeoSlug"] = game["slug"] + "-ps2-" + ("pal" if group["family"] == "PAL" else "ntsc") + "-" + region["code"].lower()
        ...
        ps2.append(game); by_game[gid] = game
    catalog.extend(ps2)
    ...
    profiles = {g["id"]: details[g["id"]]["ps2Edition"] for g in ps2}
    ...
    for gid, profile in profiles.items():
        details[gid]["ps2Edition"] = {"schemaVersion": 2, "status": profile["status"], "releaseId": profile["releaseId"], "workId": profile["workId"]}
    ...
    dump_gzip(ROOT / "data/ps2-works.json.gz", {"schemaVersion": 1, "works": active_works})
    dump(ROOT / "data/ps2-region-markets.json", {"schemaVersion": 1, "markets": region_registry})
    with (ART / "PS2-REGION-MAP.csv").open("w", newline="") as output:
    ...
    summary = {"before": dict(Counter(g["region"] for g in baseline["catalog"])), "after": dict(Counter(g["region"] for g in ps2)), "actions": dict(Counter(r["action"] for r in rows)), "afterTotal": len(ps2), "sourcedCovers": len(approved_assets), "withLanguages": sum(bool(g["languages"]) for g in ps2), "withMultipleLanguages": sum(len(g["languages"]) > 1 for g in ps2), "multidiscEditions": sum(len(profiles[g["id"]]["components"]) > 1 for g in ps2), "works": len(active_works), "preservedUrls": len(baseline["catalog"]), "urlChanges": 0}
