"""PS2 evidence resolution. A serial identifies candidates, never a unique box."""
...
def title_key(value):
    ...
    text = re.sub(r"^\.\s*hack\b", "Dot Hack", text, flags=re.I)
...
def page_languages(page, redump):
    ...
    menu_text = re.split(r";\s*(?:voices?|audio)|\b(?:with|and)\s+(?:voices?|audio)", menu_text, flags=re.I)[0]
    ...
    claims = [{"value": tags, **evidence("psxdatacenter-index", "https://psxdatacenter.com/psx2/plist2.html" if "pal" in page["sourceRegionFamilies"] else page["url"], "documented")}, {"value": detailed, "raw": text, **evidence("psxdatacenter-detail", page["url"], "documented")}]
    if rd:
        claims.append({"value": rd, **evidence(redump[0]["source"], redump[0]["sourceUrl"], "high")})
    # Index flags can describe the text localization while the game also has
    # separate voice languages (for example FFX ES text / EN voices).
    index_difference = bool(tags and all_languages and set(tags) not in [set(all_languages), set(menus)])
    return {"all": all_languages, "text": menus, "audio": audio, "index": tags, "discrepancy": index_difference or bool(rd and set(all_languages) != set(rd)), "claims": claims}
...
def build_reference():
    ...
    for row in load_gzip("psx-index.json.gz"):
        if any(url in {p["url"] for p in pages} for url in row["infoUrls"]):
            continue
    ...
    # PS2 starts without inferred label/disc aliases.
    aliases = []
    ...
    for page in pages:
        ...
        names += [alias.strip() for title in page["commonTitles"] for alias in title.split(" / ") if alias.strip()]
        ...
        reasons = []
        warnings = []
        release = dict(page["releaseDateCandidate"])
        # Sony's platform chronology places the first PS2 retail launch on
        # 2000-03-04. Keep earlier source claims, but do not publish them as a
        # normalized commercial release date (Orphen has a 1999 source typo).
        # https://sonyinteractive.com/en/press-releases/2003/playstation2-achieves-cumulative-worldwide-shipment-of-50-million-units/
        if release.get("iso") and release["iso"] < "2000-03-04":
            warnings.append("release_date_precedes_ps2_retail_launch")
            release["iso"] = None
        ...
        if retail and not any(title_key(r["title"]) in titles for r in preferred):
            reasons.append("source_title_and_redump_serial_title_disagree")
