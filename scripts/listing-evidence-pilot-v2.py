...
def process_case(case: dict[str, Any], *, output_dir: Path, cache_dir: Path, by_id: dict[str, dict[str, Any]], by_title: dict[tuple[str, str], list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    result = {
        ...
        "subjectAcceptedImages": sum(1 for row in classifications if row.get("subject") == ACCEPTED_SUBJECT),
        "componentClassifiedImages": sum(1 for row in classifications if row.get("subject") == ACCEPTED_SUBJECT and row.get("component") != "UNKNOWN_COMPONENT"),
...
def funnel_markdown(results: list[dict[str, Any]], *, selected: int, hydrated: int) -> str:
    ...
    lines = [
        ...
        f"- original-resolution images: {total('originalResolutionPhotos')}", f"- photos triaged: {total('photosClassified')}",
        f"- subject accepted: {total('subjectAcceptedImages')}", f"- component classified: {total('componentClassifiedImages')}",
        f"- high-detail inspected: {total('photosInspectedHighDetail')}", f"- component-bound observations: {total('componentBoundObservations')}",
        f"- market-bound observations: {total('marketBoundObservations')}", "- resolved cases: pending final resolver", "",
        "## Loss audit", "",
