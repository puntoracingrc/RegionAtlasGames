...
La autorización del pipeline incluye crear, guardar y publicar la ficha mediante Admin, así como aceptar el diálogo final de confirmación. Codex realiza esas acciones directamente y no solicita una aprobación final adicional del usuario para cada juego.

La verificación pública forma parte de la misma incorporación. Si la publicación deja datos confirmados sin guardar, separa incorrectamente fichas, ediciones o regiones, falla al vincular assets, produce duplicados o revela otro defecto reproducible, Codex debe corregir el fallo y volver a verificar la ficha antes de consumir el resultado o preparar el siguiente juego. Primero usa la edición y las herramientas existentes de Admin; si el problema es un fallo general del administrador o del runtime, realiza una corrección general con pruebas mediante el flujo de código habitual. Nunca crea un parche de catálogo específico para ese juego ni adopta cambios de arquitectura sugeridos por la investigación externa.

La cadena sólo se pausa cuando la corrección segura requiere una decisión real de producto o arquitectura, puede afectar datos ajenos, o existe un bloqueo externo que Codex no puede resolver. Un fallo corregible de validación, subida, agrupación, edición o publicación no requiere una nueva autorización del usuario.
...
Sólo se toca el repositorio cuando se ha demostrado un fallo real del pipeline, del administrador o del código mediante una reproducción concreta. Un resultado de investigación no autoriza por sí mismo cambios de reglas, schemas, arquitectura, interfaz o modelo de datos. Si un dato confirmado no puede representarse con el administrador actual, primero se reproduce la carencia y, si es un defecto general corregible, se repara para todas las fichas afectadas con sus pruebas. Sólo se registra como pendiente y se pausa la cadena cuando la solución exige una decisión real de producto o arquitectura; nunca se sustituye el flujo administrativo por un commit específico para ese juego.
