...
export default async function CollectionOutOfScopePlatformPage({ params }: Props) {
  ...
  const { items, normalizedSlug } = await loadGapPlatformItems(user.id, "outOfScope", platform);
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-[1600px] px-4 py-8 md:px-6">
        <CollectionGapPlatformDetail
          variant="outOfScope"
          platformSlug={normalizedSlug}
          items={items}
        />
        {items.length === 0 && (
          <p className="mt-4 text-center text-sm text-muted">
            No hay juegos fuera de catálogo en {title}.{" "}
