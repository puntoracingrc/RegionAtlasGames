...
export default async function AwardsPage() {
  ...
  return <AwardPageShell title="Premios" description="Obras, equipos y trayectorias que han marcado la historia del videojuego.">
    <section className="py-7"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-bold">Últimos ganadores</h2><Link href="/premios/ultimos-ganadores" className="text-sm font-semibold text-accent">Ver ganadores <ArrowUpRight className="inline h-4 w-4" aria-hidden="true" /></Link></div>
      <div className="grid items-start gap-6 lg:grid-cols-3">{[...workGroups].map(([key,results]) => <article key={key} className="min-w-0 border-t border-border pt-5"><AwardRecipient recipient={results[0].recipients[0]} image resultId={results[0].id} /><AwardResultList results={results} recipients={false} /></article>)}</div>
    </section>
    {!!upcoming.length && <section className="border-t border-border py-7"><h2 className="text-xl font-bold">Próximas ceremonias</h2><ul className="mt-4 divide-y divide-border">{upcoming.map(e => <li key={e.id} className="grid grid-cols-[minmax(0,1fr)_11ch] items-center gap-3 py-3"><Link href={`/premios/${e.seriesSlug}/${e.editionYear}`} className="min-w-0 break-words font-semibold text-accent">{getAwardSeriesView(e.seriesSlug)!.series.canonicalName} {e.editionYear}</Link><time className="whitespace-nowrap text-center tabular-nums" dateTime={e.ceremonyDate!}>{e.ceremonyDate}</time></li>)}</ul></section>}
