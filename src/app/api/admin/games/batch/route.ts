...
async function processBatch(request: Request) {
  ...
  for (const row of [selection.row]) {
    if (row.existingCatalogId) {
      ...
      existingDrafts.push({
        ...
        draft: {
          ...
          coverUrl: resolveRegionalBatchCoverUrl(row.group.coverUrl, body.coverUrl, current.coverUrl, body.groups.length),
    ...
    const draft = draftFromManualInput({
      ...
      coverUrl: resolveRegionalBatchCoverUrl(row.group.coverUrl, body.coverUrl, null, body.groups.length),
      year,
