  ...
  return (
    <Panel>
      ...
      <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2">
        ...
        <div><dt className="font-semibold">EAN de esta caja</dt><dd className="mt-1 font-mono text-muted">{scans.packaging.ean}</dd></div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {scans.images.map(asset => <figure key={asset.url} className="min-w-0 rounded-lg border border-border p-2">
            <a href={asset.url} target="_blank" rel="noopener noreferrer" aria-label={`Ampliar ${asset.label.toLowerCase()} de ${title}`}>
              <Image unoptimized src={asset.thumbnailUrl} width={asset.width} height={asset.height} alt={`${asset.label} de ${title} · PAL España`} className="h-44 w-full object-contain" />
            </a>
