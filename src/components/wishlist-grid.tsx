...
export function WishlistGrid({ games }: { games: WishlistCard[] }) {
  ...
  const visible = games.filter((game) => `${game.title} ${game.physicalVariantLabel ?? ""} ${game.platformName} ${game.regions.join(" ")}`.toLocaleLowerCase("es").includes(query.trim().toLocaleLowerCase("es")));
  ...
  async function change(game: WishlistCard, acquire: boolean) {
    const identityKey = game.physicalVariantId ? `physical:${game.physicalVariantId}` : `catalog:${game.catalogId}`;
    setBusy(identityKey);
    setError(null);
    try {
      const response = await fetch(acquire ? "/api/user/collection/items" : "/api/user/wishlist", {
        method: acquire ? "POST" : "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ catalogId: game.catalogId, physicalVariantId: game.physicalVariantId }),
      });
  ...
  return <div className="space-y-5">
    ...
    <div ref={root} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {pageGames.map((game) => {
        const identityKey = game.physicalVariantId ? `physical:${game.physicalVariantId}` : `catalog:${game.catalogId}`;
        return <article key={identityKey} data-wishlist-catalog-id={identityKey} className="flex min-w-0 flex-col rounded-2xl border border-border bg-card p-4">
        ...
        {game.physicalVariantLabel ? <p className="mt-1 text-xs font-semibold text-foreground/80">{game.physicalVariantLabel}</p> : null}
        <div className="mb-4 mt-2 flex flex-wrap gap-2 text-xs">{game.regions.map((region) => <RegionFlag key={region} region={region} size="sm" showLabel />)}</div>
        ...
        <div className="mt-auto flex gap-2">
          <button type="button" onClick={() => change(game, true)} disabled={Boolean(busy) || refreshing} className="btn-primary inline-flex min-h-11 flex-1 items-center justify-center gap-2 disabled:opacity-50"><Check aria-hidden className="h-4 w-4" />{busy === identityKey ? "Guardando…" : "Ya lo tengo"}</button>
          <button type="button" onClick={() => change(game, false)} disabled={Boolean(busy) || refreshing} aria-label={`Quitar ${game.title} de mis deseados`} className="rounded-lg border border-border px-3 text-muted transition hover:bg-card-hover hover:text-foreground disabled:opacity-50"><Trash2 aria-hidden className="h-4 w-4" /></button>
        </div>
      </article>;
      })}
    </div>
