        ...
        <div className="min-w-0 flex-1 space-y-3">
          <div>
            ...
            <p className="mt-2 text-foreground/85">
              {view.editorialOnly ? (
                <>Entidad editorial documentada en {view.platformHistoryLinks.length} {view.platformHistoryLinks.length === 1 ? "historia de plataforma" : "historias de plataforma"}</>
              ) : (
                <>
                  ...
                  {view.developerCatalogEntryCount > 0 && <> · {formatCatalogEntryCount(view.developerCatalogEntryCount)} como desarrolladora</>}
                  {view.publisherCatalogEntryCount > 0 && <> · {formatCatalogEntryCount(view.publisherCatalogEntryCount)} como publicadora</>}
                  {view.digitalPublisherCatalogEntryCount > 0 && <> · {formatCatalogEntryCount(view.digitalPublisherCatalogEntryCount)} como editora digital</>}
                  {view.physicalPublisherCatalogEntryCount > 0 && <> · {formatCatalogEntryCount(view.physicalPublisherCatalogEntryCount)} en edición o distribución física</>}
                </>
              )}
            </p>
          </div>
