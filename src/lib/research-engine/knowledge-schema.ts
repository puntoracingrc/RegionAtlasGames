...
export function parseResearchSources(value: unknown): ResearchSourceDefinition[] {
  ...
  return root.sources.map((raw, index) => {
    ...
    return {
      ...
      accessModes,
      fieldCapabilities,
      discoveryCapabilities: Array.isArray(row.discoveryCapabilities) ? stringArray(row.discoveryCapabilities, `sources[${index}].discoveryCapabilities`) : [],
      confirmationCapabilities: Array.isArray(row.confirmationCapabilities) ? stringArray(row.confirmationCapabilities, `sources[${index}].confirmationCapabilities`) : [],
      negativeEvidenceCapabilities: Array.isArray(row.negativeEvidenceCapabilities) ? stringArray(row.negativeEvidenceCapabilities, `sources[${index}].negativeEvidenceCapabilities`) : [],
...
export function sourcesFromPlatformKnowledge(knowledge: ResearchPlatformKnowledge): ResearchSourceDefinition[] {
  return Object.entries(knowledge.sources).map(([id, raw]) => {
    ...
    return {
      ...
      fieldCapabilities,
      discoveryCapabilities: Array.isArray(raw.discoveryCapabilities) ? raw.discoveryCapabilities.filter((item): item is string => typeof item === "string") : [],
      confirmationCapabilities: Array.isArray(raw.confirmationCapabilities) ? raw.confirmationCapabilities.filter((item): item is string => typeof item === "string") : [],
      negativeEvidenceCapabilities: Array.isArray(raw.negativeEvidenceCapabilities) ? raw.negativeEvidenceCapabilities.filter((item): item is string => typeof item === "string") : [],
