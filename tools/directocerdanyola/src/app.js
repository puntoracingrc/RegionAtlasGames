...
function applyCircuit(circuit){
  if(!circuit||!circuit.map||!circuit.map.guidePath)return;const svg=$('trackMap'),photoCircuit=L.normalize(circuit.id||circuit.name).includes('CERDANYOLA'),map=photoCircuit?defaultCircuit.map:circuit.map;activeCircuit={...circuit,map};svg.classList.toggle('photo-circuit',photoCircuit);$('trackMarkers').setAttribute('transform',photoCircuit?'translate(0 -10)':'');svg.setAttribute('viewBox',map.viewBox||'0 0 1000 390');$('trackMapSvgTitle').textContent=map.title||circuit.name;$('trackMapSvgDesc').textContent=map.description||`Trazado archivado de ${circuit.name}.`;$('circuitGuide').setAttribute('d',map.guidePath);
  const infields=svg.querySelector('.track-infields');if(infields)infields.replaceChildren(...(map.infieldPaths||[]).map(d=>svgElement('path',{d})));
  ...
  const directions=svg.querySelector('.direction-guide');if(directions)directions.replaceChildren(...(map.directionPaths||[]).map(d=>svgElement('path',{d,'marker-end':'url(#directionArrow)'})));
...
let voiceSettings=loadVoiceSettings(),voiceEnabled=false,voiceAvailable=false,voiceQueue=[],voiceBusy=false,voicePaused=false,voiceGeneration=0,voiceCurrentAudio=null,voiceCurrentUrl='';
...
function shouldSpeak(item){return item.spoken!==false&&voiceEnabled&&voiceAvailable&&!voicePaused&&voiceSettings.types[item.voiceType||'progress']!==false&&(voiceSettings.mode!=='pilot'||voiceItemMatchesPilot(item));}
...
function clearVoiceQueue(stop=false){
  voiceQueue=[];if(stop)voiceGeneration+=1;if(stop&&voiceCurrentAudio){const audio=voiceCurrentAudio;audio.pause();if(typeof audio.onended==='function')audio.onended();audio.src='';voiceCurrentAudio=null;}if(stop&&voiceCurrentUrl){URL.revokeObjectURL(voiceCurrentUrl);voiceCurrentUrl='';}if(stop||!voiceCurrentAudio)voiceBusy=false;if(voiceEnabled)setVoiceStatus(voicePaused?'Voz pausada con la carrera':voiceCurrentAudio?'Terminando el aviso actual':'Escuchando nuevos avisos');
}
async function playVoiceQueue(){
  if(voiceBusy||voicePaused||!voiceEnabled||!voiceQueue.length)return;voiceBusy=true;const generation=voiceGeneration,item=voiceQueue.shift();let source='',audio=null;setVoiceStatus(`Hablando · ${item.kind.toLowerCase()}`);
  try{const blob=await item.audioPromise;if(generation!==voiceGeneration||voicePaused||!voiceEnabled)return;source=URL.createObjectURL(blob);audio=new Audio(source);voiceCurrentAudio=audio;voiceCurrentUrl=source;audio.volume=voiceSettings.volume;await new Promise((resolve,reject)=>{audio.onended=resolve;audio.onerror=()=>reject(new Error('No se pudo reproducir el audio.'));audio.play().catch(reject);});}
  catch(error){console.warn('Aviso de voz omitido',error);setVoiceStatus(error.message||'Aviso de voz omitido',true);}
  finally{if(source)URL.revokeObjectURL(source);if(voiceCurrentUrl===source)voiceCurrentUrl='';if(voiceCurrentAudio===audio)voiceCurrentAudio=null;if(generation===voiceGeneration){voiceBusy=false;if(voiceEnabled)setVoiceStatus(voicePaused?'Voz pausada con la carrera':voiceQueue.length?`${voiceQueue.length} avisos pendientes`:'Escuchando nuevos avisos');if(!voicePaused&&voiceQueue.length)void playVoiceQueue();}}
}
function setVoicePlaybackPaused(value){
  voicePaused=Boolean(value);if(voicePaused){clearVoiceQueue(true);if(voiceEnabled)setVoiceStatus('Voz pausada con la carrera');return;}if(voiceEnabled){setVoiceStatus('Escuchando nuevos avisos');void playVoiceQueue();}
}
...
async function previewSelectedVoice(){
  ...
  finally{if(voiceCurrentUrl)URL.revokeObjectURL(voiceCurrentUrl);voiceCurrentUrl='';voiceCurrentAudio=null;voiceBusy=false;$('voicePreview').disabled=false;if(!failed)setVoiceStatus(voiceEnabled?(voicePaused?'Voz pausada con la carrera':'Escuchando nuevos avisos'):'Voz desactivada');if(!voicePaused&&voiceQueue.length)void playVoiceQueue();}
}
async function toggleVoiceNarration(){
  ...
  if(!voiceAvailable){await checkVoiceAvailability();if(!voiceAvailable)return;}voiceEnabled=true;renderVoiceSettings();setVoiceStatus(voicePaused?'Voz pausada con la carrera':'Escuchando nuevos avisos');if(!voicePaused)enqueueVoiceNarration({kind:'SISTEMA',voiceType:'start',priority:10,text:voiceSettings.mode==='pilot'?'Radio para piloto activada.':'Narración de carrera activada.',pilots:['system']});
}
function resetLiveNarrative(context=''){
  liveNarrativeContext=context;liveNarrativeEntries=[];battleNarrativeState={};liveNarrativeLast.clear();narrativeDriverStates.clear();voiceLastSpoken.clear();clearVoiceQueue(false);renderLiveNarrative();
}
...
function recordLiveNarrative(items){
  ...
  for(const item of items){
    if(!item||!item.text)continue;const previous=liveNarrativeLast.get(item.key),cooldown=Number(item.cooldown)||0;if(previous&&(item.once||previous.text===item.text))continue;if(previous&&cooldown&&now-previous.at<cooldown)continue;
...
function beginDemoPlayback(){
  cancelDemoCountdown();demoStartedAt=Date.now();demoPlaying=true;setVoicePlaybackPaused(false);setConnection('live','Reproduciendo carrera');updateDemoControls();
}
function startDemoCountdown(){
  cancelDemoCountdown();setVoicePlaybackPaused(true);demoCountdownValue=3;$('demoCountdownValue').textContent='3';$('demoCountdown').hidden=false;setConnection('paused','La repetición empieza en 3…');updateDemoControls();
...
function renderDemoFrame(force=false){
  ...
  const signature=enriched.map(driver=>{const state=demoFrameProgress.get(String(driver.key));return `${driver.key}:${driver.position}:${driver.laps}:${driver.lastLap}:${driver.realLapDeficit||0}:${Boolean(state&&state.startDelayActive)}:${Boolean(state&&state.didNotStart)}:${Boolean(state&&state.refueling)}:${Boolean(state&&state.missingCrossing)}:${Boolean(state&&state.hiddenAfterNoCrossing)}`;}).join('|');
...
function toggleDemoPlayback(){
  if(!demoMode)return;
  if(demoCountdownValue){cancelDemoCountdown();setVoicePlaybackPaused(true);setConnection('paused','Repetición preparada');return;}
  if(demoCurrentElapsed()>=demoReplay.durationSeconds){restartDemo();startDemoCountdown();return;}
  if(demoPlaying){demoElapsedBase=demoCurrentElapsed();demoPlaying=false;demoStartedAt=0;setVoicePlaybackPaused(true);setConnection('paused','Repetición pausada');}
  else startDemoCountdown();
  updateDemoControls();
}
function restartDemo(){if(!demoMode)return;setVoicePlaybackPaused(true);resetDemoTimeline();setConnection('paused','Repetición preparada');renderDemoFrame(true);}
...
function seekDemo(value){
  if(!demoMode)return;cancelDemoCountdown();clearVoiceQueue(true);demoElapsedBase=Math.max(0,Math.min(demoReplay.durationSeconds,Number(value)||0));demoStartedAt=demoPlaying?Date.now():0;if(demoElapsedBase>=demoReplay.durationSeconds){demoPlaying=false;demoStartedAt=0;}voicePaused=!demoPlaying;demoSignature='';demoPreviousPositions.clear();demoFrameProgress.clear();demoHeavyAt=0;demoTimingAt=0;demoUiSecond=-1;fuelVisualAt=0;resetLiveNarrative();renderDemoFrame(true);
}
function stopDemo(refresh=true){
  ...
  setVoicePlaybackPaused(paused);if(!refresh)return;
...
async function refreshReplayArchiveOptions(){
  ...
  const hasCompleteNitroDemo=eventKey==='100645';
  if(hasCompleteNitroDemo){const option=document.createElement('option');option.value='builtin';option.textContent='Demo completa · Nitro · Marc García · 30:12';select.append(option);}
  ...
  $('archiveRacePicker').hidden=select.options.length===0;const complete=replayArchiveSummaries.find(item=>item.status==='complete');return hasCompleteNitroDemo?'builtin':complete&&complete.raceId||select.options[0]&&select.options[0].value||'';
}
...
async function startDemo(){
  ...
  demoMode=true;paused=false;setVoicePlaybackPaused(true);$('pauseButton').textContent='Pausar pantalla';category=demoReplay.category==='ECO'?'ECO':'NITRO';document.body.dataset.category=category;officialRanking=[];officialRuns=[];officialSectionKey='';officialFinalComplete=false;rankingStatus='replay';demoSpeed=Number($('demoSpeed').value)||1;resetDemoTimeline();
...
function renderTrackMap(){
  ...
  if(markers.dataset.roster!==roster){markers.innerHTML=enriched.map(driver=>`<g class="track-driver" data-track-driver="${esc(driver.key)}" style="--driver-colour:${trackColour(driver)}" role="img" aria-label="Posición ${driver.position}, ${esc(driver.name)}"><circle class="driver-disc" r="24"></circle><text class="driver-initials" text-anchor="middle" dominant-baseline="central">${driverInitials(driver.name)}</text><circle class="driver-rank" cx="-20" cy="-20" r="10"></circle><text class="driver-rank-text" x="-20" y="-20" text-anchor="middle" dominant-baseline="central">${driver.position}</text><title>${driver.position}. ${esc(driver.name)}</title></g>`).join('');markers.dataset.roster=roster;}
...
function timingMeta(driver,index,battleMap){
  ...
  return {driver,index,trendText,trendClass,signal,fuel,battleClass,battleGap,battleGapLabel,chips:`${pitChip}${startChip}${trackingChip}${lappedChip}`,gap,previousGap,sessionFastest,rowClass:`${rowClass} ${realLapDeficit?'lapped-real':''}`.trim(),matched:driver.matchedPilot?`Campeonato: ${driver.matchedPilot.shortName}`:'Fuera de la general cargada'};
}
...
function syncTimingRow(row,meta){
  ...
  const driverCell=row.querySelector('.driver-cell');if(driverCell){const copy=driverCell.querySelector('.driver-copy')||driverCell,name=copy.querySelector(':scope > strong');if(name)name.textContent=driver.name;copy.querySelectorAll('.battle-chip,.pit-chip,.start-delay-chip,.tracking-chip,.lapped-chip').forEach(node=>node.remove());const meter=copy.querySelector('.pilot-meter');if(meter&&chips)meter.insertAdjacentHTML('beforebegin',chips);const pace=copy.querySelector('.pace-track i');if(pace)pace.style.width=`${driver.pace}%`;syncFuelTank(driverCell,fuel);}
...
function gapNarrative(value){const text=String(value||'');if(!text)return '';const lapMatch=text.match(/^-([0-9]+)$/);if(lapMatch){const laps=Number(lapMatch[1]);return laps===1?'una vuelta':`${laps} vueltas`;}const seconds=L.gapSeconds(text);return seconds==null?'':`${decimal(seconds,3)} segundos`;}
function leaderGapNarrative(){const second=enriched[1];return second?gapNarrative(second.gapFirst||second.gapPrevious):'';}
...
function renderBroadcast(){
  ...
  const insights=[];const leader=enriched[0],leadGap=leaderGapNarrative(),fastest=[...enriched].filter(d=>d.bestSeconds).sort((a,b)=>a.bestSeconds-b.bestSeconds)[0];
  if(leader)insights.push(`${leader.name} lidera con ${leader.laps} vueltas${leadGap?` y ${leadGap} sobre el segundo`:''}.`);
  ...
  const activeBattleGroups=L.battleGroups(enriched,battleThresholdSeconds),closeBattle=activeBattleGroups[0];
  ...
  const narrative=[],elapsed=raceElapsedSeconds(),battleTransition=L.battleNarrativeTransitions(enriched,activeBattleGroups,battleNarrativeState,elapsed,3),running=elapsed>0||/RUN/i.test(snapshot.raceState||''),finished=archiveFrameFinished(archiveFrame(snapshot)),opening=running&&elapsed<=45&&Math.max(0,...enriched.map(driver=>Number(driver.laps)||0))<=2,hasPreviousResults=L.hasPreviousChampionshipResults(seeds[category]);battleNarrativeState=battleTransition.state;
  if(opening){
    const startingGridDrivers=demoMode?D.frame(demoReplay,0).drivers:enriched;
    narrative.push({key:'race-start',kind:'SALIDA',voiceType:'start',priority:10,once:true,text:`Comienza ${snapshot.name||snapshot.group||'la prueba'}: ${snapshot.group||(category==='NITRO'?'GT8 Nitro':'GT8 Eléctrico')} en ${activeCircuit.name}.`});
    L.startingGridNarrative(startingGridDrivers,5).forEach((text,index)=>narrative.push({key:`starting-grid:${index}`,kind:'PARRILLA',voiceType:'start',priority:10,once:true,pilots:startingGridDrivers.slice(index*5,index*5+5).map(voicePilotId),text}));
  }
  ...
  if(hasPreviousResults&&elapsed>=120)for(const text of broadcast.paragraphs.slice(0,2))narrative.push({key:`championship:${text}`,kind:'CAMPEONATO',voiceType:'championship',priority:1,text});
  if(leader&&leader.laps)narrative.push({key:`leader:${leader.key||leader.name}:${leader.laps}`,kind:'CARRERA',voiceType:'battle',spoken:leader.laps%5===0,pilots:[voicePilotId(leader)],lap:leader.laps,text:`${leader.name} lidera con ${leader.laps} vuelta${leader.laps===1?'':'s'}${leadGap?` y ${leadGap} sobre el segundo`:''}.`});
  ...
  for(const event of battleTransition.events){const pilots=[voicePilotId(event.ahead),voicePilotId(event.behind)],lap=Math.max(Number(event.ahead.laps)||0,Number(event.behind.laps)||0),place=`${event.position}.ª plaza`,names=`${event.ahead.name} y ${event.behind.name}`;let text='';if(event.type==='start')text=`Lucha por la ${place}: ${names}, separados por ${decimal(event.gap,3)} segundos.`;else if(event.type==='continue')text=`Continúan ${names} luchando por la ${place}; la distancia baja de ${decimal(event.fromGap,3)} a ${decimal(event.gap,3)} segundos.`;else if(event.type==='pass')text=`${event.ahead.name} consigue arrebatar la ${place} a ${event.behind.name}.`;else if(event.type==='held')text=`${event.ahead.name} consigue mantener la ${place} y alejarse de ${event.behind.name}; abre ${decimal(event.gap,3)} segundos.`;if(text)narrative.push({key:`battle:${event.type}:${event.key}:${Math.floor(elapsed/10)}`,kind:event.type==='pass'?'ADELANTAMIENTO':event.type==='held'?'POSICIÓN':'LUCHA',voiceType:'battle',priority:event.type==='pass'?8:6,pilots,lap,once:true,text});}
  if(leader)for(const driver of enriched.slice(1).filter(item=>Number(item.realLapDeficit)>0)){const deficit=Number(driver.realLapDeficit);narrative.push({key:`lapped:${leader.key||leader.name}:${driver.key||driver.name}:${deficit}`,kind:'DOBLAJE',voiceType:'battle',priority:driver.position<=3?8:4,spoken:driver.position<=3,once:true,pilots:[voicePilotId(leader),voicePilotId(driver)],lap:leader.laps,text:`${leader.name} completa el doblaje en pista sobre ${driver.name}, que ocupa el puesto ${driver.position}.`});}
...
$('pauseButton').addEventListener('click',()=>{paused=!paused;setVoicePlaybackPaused(paused);$('pauseButton').textContent=paused?'Reanudar pantalla':'Pausar pantalla';setConnection(paused?'paused':'live',paused?'Pantalla y voz pausadas':'MyRCM conectado');if(!paused&&queuedSnapshot){const next=queuedSnapshot;queuedSnapshot=null;applySnapshot(next,true);}});
