...
function applyCircuit(circuit){
  if(!circuit||!circuit.map||!circuit.map.guidePath)return;activeCircuit=circuit;const map=circuit.map,svg=$('trackMap'),photoCircuit=L.normalize(circuit.id||circuit.name).includes('CERDANYOLA');svg.classList.toggle('photo-circuit',photoCircuit);svg.setAttribute('viewBox',map.viewBox||'0 0 1000 390');$('trackMapSvgTitle').textContent=map.title||circuit.name;$('trackMapSvgDesc').textContent=map.description||`Trazado archivado de ${circuit.name}.`;$('circuitGuide').setAttribute('d',map.guidePath);
